...
Try {
    ...
    [String]$appName = 'Microsoft Access 365'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        If($32BitFilePath.Exists) {
			# Install Microsoft Access 365
			Execute-Process -Path "$32BitFilePath" -Parameters "/configure Access365.xml" -WindowStyle Hidden
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		$AnyFilePath = Get-ChildItem -Path "$dirFiles" -Include UninstallAccess365.xml -File -Recurse -ErrorAction SilentlyContinue
        If($AnyFilePath.Exists) {
			...
			If($SourceCache.Exists) {
				Execute-Process -Path "$SourceCache" -Parameters "/configure UninstallAccess365.xml" -WindowStyle Hidden -IgnoreExitCodes "-1"
			}
        }
